...
6. browser_screenshot — 截图。参数: path（可选）, selector（可选）
7. browser_text — 提取网页文字。参数: selector（可选）
8. browser_click — 点击元素。参数: selector（必填）
9. browser_type — 输入文字。参数: selector（必填）, text（必填）
10. browser_assert — 断言验证。参数: type（必填, 可选值: element_exists, element_not_exists, text_contains, text_equals, url_contains, url_equals, value_contains, value_equals）, selector（可选）, attribute（可选）, expected（可选）
11. browser_wait — 等待条件。参数: type（必填, 可选值: element_visible, element_hidden, text_visible, url_contains）, selector（可选）, expected（可选）, timeout（可选, 默认10000ms）
12. browser_eval — 执行JS代码。参数: expression（必填）
13. git_status — 查看git状态。参数: 无
14. git_diff — 查看文件差异。参数: file（可选）, staged（可选, boolean）
15. git_commit — 创建提交。参数: message（必填）, files（可选, 空格分隔的文件路径）
16. git_push — 推送到远程。参数: remote（可选, 默认origin）, branch（可选, 默认当前分支）
...
[TOOL_CALL:git_diff(file="src/app.ts")]
[TOOL_CALL:git_commit(message="feat: add new feature", files="src/app.ts src/utils.ts")]
[TOOL_CALL:git_push(remote="origin", branch="main")]
